...
import { useAuth, useDmMessages, sendDmMessage } from '@maskord/shared';
import type { DirectMessage } from '@maskord/shared';
...
type Route = RouteProp<FriendsStackParamList, 'DmChat'>;

export default function DmChatScreen() {
  const route = useRoute<Route>();
  const { partnerUid } = route.params;
  const { firebaseUser } = useAuth();

  const { messages, loading, hasMore, loadMore } =
    useDmMessages(firebaseUser?.uid ?? null, partnerUid);
  ...
  async function handleSend() {
    ...
    setInput('');
    await sendDmMessage(firebaseUser.uid, partnerUid, content);
  }

  function renderMessage({ item }: { item: DirectMessage }) {
    const isOwn = (item.senderId ?? item.authorId) === firebaseUser?.uid;
