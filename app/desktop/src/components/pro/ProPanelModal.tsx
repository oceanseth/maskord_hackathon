...
/**
 * Subscription status and the rent-out shelf, in one place.
 *
 * This is our stand-in for RevenueCat's Customer Center, which is iOS/Android
 * only: everything here comes from `CustomerInfo`, including the management URL
      ...
      <div className="px-6 pb-6 space-y-5">
        ...
        <section className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-white">
              {pro.pro ? 'Pro is active' : pro.lapsed ? 'Pro has expired' : 'Not subscribed'}
            </span>
            {!pro.pro && (
              <button
                onClick={onUpgrade}
        ...
        <section className="space-y-2 border-t border-[#1e1e2e] pt-4">
          ...
          <p className="text-xs text-[#6b7280]">
            A published mask can be seated by any Pro member. You are credited every time it
            takes a seat — the count is what a payout would be calculated from.
          </p>
