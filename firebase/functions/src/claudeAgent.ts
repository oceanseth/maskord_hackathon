...
export const onClaudeTranscriptUtterance = onDocumentCreated(
  ...
  async (event) => {
    ...
    // masky is used purely as the voice. This is unconditional: an avatar that
    // can use tools only when someone happens to be sharing a screen is not one
    // you can rely on, so voice now behaves like a text channel — full history,
    // live presence, every tool, every turn.
    const presence   = await loadVoicePresence(guildId);
